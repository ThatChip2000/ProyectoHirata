...
public class RegistroKilometraje extends JFrame {
    private static final String DB_URL = "jdbc:mysql://localhost:3306/hirata_flotas?serverTimezone=UTC&useUnicode=true&characterEncoding=UTF-8";
    ...
    private JComboBox<EquipoItem> equipoComboEq;
    ...
    private JRadioButton mantSoftwareBtnEq;
    ...
    private JPanel areaDinamicaEq;
    private JTextArea correctivoArea;
    private JPanel softwarePanelEq;
    private DefaultTableModel softwareTableModel;
    private ButtonGroup grupoMantEq;

    // Campos para módulo de consulta de mantenciones de equipos
    private JComboBox<EquipoItem> consultaEquipoComboEq;
    ...
    private JPanel crearPanelMantenimientoEquipos() {
        ...
        equipoComboEq = new JComboBox<>();
        equipoComboEq.setFont(new Font("Segoe UI", Font.PLAIN, 11));
        equipoComboEq.setPreferredSize(new Dimension(220, 26));
        cargarEquiposEnCombo();

        JButton agregarEquipoBtn = new JButton("+ Agregar Equipo");
        agregarEquipoBtn.setFont(boldFont);
        agregarEquipoBtn.setBackground(new Color(40, 140, 70));
        agregarEquipoBtn.setForeground(Color.WHITE);
        agregarEquipoBtn.setFocusPainted(false);

        c1.gridx = 0; c1.gridy = 0;
        seccion1.add(new JLabel("Equipo:"), c1);
        c1.gridx = 1;
        seccion1.add(equipoComboEq, c1);
        c1.gridx = 2;
        seccion1.add(agregarEquipoBtn, c1);

        equipoInfoLabel = new JLabel("Selecciona un equipo de la lista...");
        ...
        mantSoftwareBtnEq    = new JRadioButton("Software");
        mantSoftwareBtnEq.setVisible(false); // solo visible para PCs

        grupoMantEq = new ButtonGroup();
        grupoMantEq.add(mantPreventivoBtnEq);
        grupoMantEq.add(mantCorrectivoBtnEq);
        grupoMantEq.add(mantSoftwareBtnEq);
        ...
        c3.gridx = 2;
        seccion3.add(mantSoftwareBtnEq, c3);
        c3.gridx = 0; c3.gridy = 1;
        c3.gridwidth = 3;
        seccion3.add(fechaHoyLabel, c3);
        c3.gridwidth = 1;

        // ===== SECCIÓN 4: ÁREA DINÁMICA (checklist / correctivo / software) =====
        areaDinamicaEq = new JPanel(new BorderLayout());
        areaDinamicaEq.setBackground(new Color(245, 245, 245));
        areaDinamicaEq.setBorder(BorderFactory.createTitledBorder("4. Detalle de Mantenimiento"));
        areaDinamicaEq.setPreferredSize(new Dimension(900, 320));

        // Sub-panel CHECKLIST (preventivo)
        ...
        checklistWrapper.setOpaque(false);
        checklistWrapper.add(checklistScrollEq, BorderLayout.CENTER);
        JPanel infoBar = new JPanel(new FlowLayout(FlowLayout.LEFT, 10, 3));
        infoBar.setOpaque(false);
        infoBar.add(progresoBarra);
        infoBar.add(itemsCriticosLabel);
        checklistWrapper.add(infoBar, BorderLayout.SOUTH);

        // Sub-panel CORRECTIVO
        JPanel correctivoWrapper = new JPanel(new BorderLayout());
        correctivoWrapper.setOpaque(false);
        correctivoWrapper.setBorder(BorderFactory.createEmptyBorder(8, 8, 8, 8));
        JLabel correctivoLbl = new JLabel("Describe el problema detectado y las acciones correctivas realizadas:");
        correctivoLbl.setFont(new Font("Segoe UI", Font.PLAIN, 11));
        correctivoArea = new JTextArea(10, 60);
        correctivoArea.setLineWrap(true);
        correctivoArea.setWrapStyleWord(true);
        correctivoArea.setFont(new Font("Segoe UI", Font.PLAIN, 11));
        correctivoArea.setBorder(BorderFactory.createCompoundBorder(
            BorderFactory.createLineBorder(new Color(180, 180, 180)),
            BorderFactory.createEmptyBorder(6, 8, 6, 8)));
        correctivoWrapper.add(correctivoLbl, BorderLayout.NORTH);
        correctivoWrapper.add(new JScrollPane(correctivoArea), BorderLayout.CENTER);

        // Sub-panel SOFTWARE
        softwarePanelEq = construirPanelSoftware();

        // Mostrar checklist por defecto
        areaDinamicaEq.add(checklistWrapper, BorderLayout.CENTER);
        String[] tiposVistaTag = {"checklist"};  // índice mutable

        // ── Listeners de radio buttons ──────────────────────────────────────
        java.awt.event.ActionListener radioListener = e -> {
            areaDinamicaEq.removeAll();
            if (mantPreventivoBtnEq.isSelected()) {
                areaDinamicaEq.add(checklistWrapper, BorderLayout.CENTER);
                tiposVistaTag[0] = "checklist";
            } else if (mantCorrectivoBtnEq.isSelected()) {
                areaDinamicaEq.add(correctivoWrapper, BorderLayout.CENTER);
                tiposVistaTag[0] = "correctivo";
            } else {
                areaDinamicaEq.add(softwarePanelEq, BorderLayout.CENTER);
                tiposVistaTag[0] = "software";
            }
            areaDinamicaEq.revalidate();
            areaDinamicaEq.repaint();
        };
        mantPreventivoBtnEq.addActionListener(radioListener);
        mantCorrectivoBtnEq.addActionListener(radioListener);
        mantSoftwareBtnEq.addActionListener(radioListener);
        ...
        sectionsPanel.add(Box.createVerticalStrut(10));
        sectionsPanel.add(areaDinamicaEq);
        sectionsPanel.add(Box.createVerticalStrut(10));
        ...
        // ===== ACTION LISTENERS =====
        equipoComboEq.addActionListener(e -> seleccionarEquipoDesdeCombo());
        agregarEquipoBtn.addActionListener(e -> mostrarDialogoAgregarEquipo());
    ...
    private JPanel crearPanelConsultaMantencionesEquipos() {
        ...
        consultaEquipoComboEq = new JComboBox<>();
        consultaEquipoComboEq.setFont(new Font("Segoe UI", Font.PLAIN, 11));
        consultaEquipoComboEq.setPreferredSize(new Dimension(220, 26));
        cargarEquiposEnComboConsulta();

        consultaEquipoInfoLabelEq = new JLabel("Selecciona un equipo de la lista para consultar sus mantenciones.");
        consultaEquipoInfoLabelEq.setFont(new Font("Segoe UI", Font.ITALIC, 10));

        c.gridx = 0;
        c.gridy = 0;
        topPanel.add(new JLabel("Equipo:"), c);
        c.gridx = 1;
        c.gridwidth = 2;
        topPanel.add(consultaEquipoComboEq, c);
        c.gridwidth = 1;
        ...
        consultaEquipoComboEq.addActionListener(e -> seleccionarEquipoDesdeComboConsulta());
    ...
    private void cargarEquiposEnCombo() {
        if (equipoComboEq == null) return;
        equipoComboEq.removeAllItems();
        equipoComboEq.addItem(new EquipoItem(-1, "— Selecciona un equipo —", "", "", "", "activo", ""));
        String sql = "SELECT e.id, e.codigo, e.tipo, e.modelo, e.ubicacion, e.estado, " +
        ...
        try (Connection conn = getConnection();
            ...
            while (rs.next()) {
                equipoComboEq.addItem(new EquipoItem(
                    rs.getInt("id"), rs.getString("codigo"), rs.getString("tipo"),
                    rs.getString("modelo"), rs.getString("ubicacion"),
                    rs.getString("estado"), rs.getString("responsable")));
            }
        } catch (SQLException ex) {
            System.err.println("Error al cargar equipos en combo: " + ex.getMessage());
        }
    }

    private void seleccionarEquipoDesdeCombo() {
        EquipoItem equipo = (EquipoItem) equipoComboEq.getSelectedItem();
        if (equipo == null || equipo.getId() == -1) {
            limpiarInfoEquipo();
            return;
        }
        ...
        equipoResponsableLabel.setText("Responsable: " +
            (equipo.getResponsable() != null && !equipo.getResponsable().isEmpty() ? equipo.getResponsable() : "-"));
        equipoInfoLabel.setText("✓ " + equipo.getCodigo() + " — " + equipo.getTipo() + " — " + equipo.getModelo());
        equipoInfoLabel.setForeground(new Color(50, 150, 50));

        boolean esPC = "computadora".equalsIgnoreCase(equipo.getTipo());
        mantSoftwareBtnEq.setVisible(esPC);
        // Si estaba en Software y cambia a un equipo no-PC, volver a Preventivo
        if (!esPC && mantSoftwareBtnEq.isSelected()) {
            mantPreventivoBtnEq.setSelected(true);
            mostrarPanelChecklist();
        }

        // Cargar el checklist si el modo actual es preventivo
        if (mantPreventivoBtnEq.isSelected()) {
            cargarChecklistEquipo(equipo.getTipo());
        }
    }

    private void mostrarPanelChecklist() {
        areaDinamicaEq.removeAll();
        JPanel checklistWrapper = new JPanel(new BorderLayout());
        checklistWrapper.setOpaque(false);
        checklistWrapper.add(checklistScrollEq, BorderLayout.CENTER);
    ...
    private void limpiarInfoEquipo() {
        equipoInfoLabel.setText("Selecciona un equipo de la lista...");
        ...
        equipoResponsableLabel.setText("Responsable: -");
        mantSoftwareBtnEq.setVisible(false);
        equipoActualEq = null;
    }
    ...
    private void guardarMantenimientoEquipo() {
        if (equipoActualEq == null) {
            JOptionPane.showMessageDialog(this, "Selecciona un equipo primero.", "Equipo requerido", JOptionPane.WARNING_MESSAGE);
            return;
        }

        String tipoMant;
        if (mantPreventivoBtnEq.isSelected())     tipoMant = "preventivo";
        else if (mantCorrectivoBtnEq.isSelected()) tipoMant = "correctivo";
        else                                       tipoMant = "software";

        // ── Validaciones según modo ───────────────────────────────────────────
        if ("correctivo".equals(tipoMant)) {
            if (correctivoArea.getText().trim().isEmpty()) {
                JOptionPane.showMessageDialog(this, "Describe las acciones correctivas realizadas.", "Campo requerido", JOptionPane.WARNING_MESSAGE);
                return;
            }
        } else if ("software".equals(tipoMant)) {
            if (softwareTableModel.getRowCount() == 0) {
                JOptionPane.showMessageDialog(this, "Agrega al menos una actualización de software.", "Campo requerido", JOptionPane.WARNING_MESSAGE);
                return;
            }
        } else {
            // preventivo: checklist
            if (checklistActualEq == null || checklistActualEq.isEmpty()) {
                JOptionPane.showMessageDialog(this, "No hay items de checklist cargados.", "Error", JOptionPane.ERROR_MESSAGE);
                return;
            }
            int completados = (int) checklistActualEq.stream().filter(ChecklistItemEstado::isCompletado).count();
            if (completados == 0) {
                JOptionPane.showMessageDialog(this, "Debes marcar al menos un item del checklist.", "Validación", JOptionPane.WARNING_MESSAGE);
                return;
            }
            int criticosPendientes = (int) checklistActualEq.stream()
                    .filter(c -> c.getItem().esCritico() && !c.isCompletado()).count();
            if (criticosPendientes > 0) {
                int confirmar = JOptionPane.showConfirmDialog(this,
                        ...
                        "Items críticos pendientes", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
                if (confirmar != JOptionPane.YES_OPTION) return;
            }
        }

        // ── Construir detalles de texto para guardar en columna detalles ──────
        String detallesTexto = "";
        if ("correctivo".equals(tipoMant)) {
            detallesTexto = correctivoArea.getText().trim();
        } else if ("software".equals(tipoMant)) {
            StringBuilder sb = new StringBuilder("Actualizaciones de software:\n");
            for (int r = 0; r < softwareTableModel.getRowCount(); r++) {
                sb.append("  • ").append(softwareTableModel.getValueAt(r, 0))
                  .append(" | ").append(softwareTableModel.getValueAt(r, 1))
                  .append(" → ").append(softwareTableModel.getValueAt(r, 2));
                Object obs = softwareTableModel.getValueAt(r, 3);
                if (obs != null && !obs.toString().isBlank()) sb.append(" (").append(obs).append(")");
                sb.append("\n");
            }
            detallesTexto = sb.toString();
        }

        String sql = "INSERT INTO mantenimiento_equipo (equipo_id, fecha_programada, tipo, estado, detalles, realizado_por_usuario_id) " +
                     "VALUES (?, ?, ?, 'completado', ?, ?)";
        ...
        try (Connection conn = getConnection()) {
            ...
            try (PreparedStatement psEquipo = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
                ...
                psEquipo.setString(4, detallesTexto.isEmpty() ? null : detallesTexto);
                psEquipo.setInt(5, usuarioIdSesion);
                psEquipo.executeUpdate();

                int idMantenimiento;
                try (ResultSet gk = psEquipo.getGeneratedKeys()) {
                    if (gk.next()) idMantenimiento = gk.getInt(1);
                    else throw new SQLException("No se obtuvo ID de mantención.");
                }

                // Guardar checklist solo en modo preventivo
                if ("preventivo".equals(tipoMant)) {
                    String sqlCk = "INSERT INTO mantenimiento_checklist (mantenimiento_equipo_id, checklist_item_id, completado, observacion, completado_en) " +
                                   "VALUES (?, ?, ?, ?, ?)";
                    try (PreparedStatement psCk = conn.prepareStatement(sqlCk)) {
                        for (ChecklistItemEstado ie : checklistActualEq) {
                            psCk.setInt(1, idMantenimiento);
                            psCk.setInt(2, ie.getItem().getId());
                            psCk.setBoolean(3, ie.isCompletado());
                            psCk.setString(4, ie.getObservacion().isEmpty() ? null : ie.getObservacion());
                            psCk.setTimestamp(5, ie.isCompletado() ? new java.sql.Timestamp(System.currentTimeMillis()) : null);
                            psCk.addBatch();
                        }
                        psCk.executeBatch();
                    }
                ...
                String resumen = "preventivo".equals(tipoMant)
                    ? "Items completados: " + checklistActualEq.stream().filter(ChecklistItemEstado::isCompletado).count() + "/" + checklistActualEq.size()
                    : detallesTexto;
            ...
            JOptionPane.showMessageDialog(this, "Error al guardar mantención:\n" + ex.getMessage(), "Error BD", JOptionPane.ERROR_MESSAGE);
        }
    }

    // ── Prefijos de código según tipo de equipo ──────────────────────────────
    private static final java.util.Map<String, String> PREFIJO_TIPO;
    static {
        PREFIJO_TIPO = new java.util.LinkedHashMap<>();
        PREFIJO_TIPO.put("computadora",  "PC");
        PREFIJO_TIPO.put("impresora",    "IMP");
        PREFIJO_TIPO.put("proyector",    "PROY");
    }

    private void mostrarDialogoAgregarEquipo() {
        JDialog dialog = new JDialog(this, "Agregar Equipo Nuevo", true);
        ...
        header.setBackground(new Color(40, 140, 70));
        JLabel headerLbl = new JLabel("Nuevo Equipo");
        ...
        JPanel form = new JPanel(new GridBagLayout());
        form.setBackground(new Color(245, 245, 245));
        form.setBorder(BorderFactory.createEmptyBorder(14, 20, 10, 20));
        GridBagConstraints gc = new GridBagConstraints();
        gc.insets = new Insets(7, 6, 7, 6);
        gc.anchor = GridBagConstraints.WEST;
        gc.fill   = GridBagConstraints.HORIZONTAL;

        Font lf = new Font("Segoe UI", Font.PLAIN, 11);

        // Tipo (ComboBox primero para generar el código)
        String[] tiposDisponibles = PREFIJO_TIPO.keySet().toArray(new String[0]);
        JComboBox<String> tipoCombo = new JComboBox<>(tiposDisponibles);
        tipoCombo.setFont(lf);

        // Código: se genera automáticamente, solo lectura
        JTextField codigoField = new JTextField(14);
        codigoField.setEditable(false);
        codigoField.setBackground(new Color(230, 230, 230));
        codigoField.setFont(new Font("Segoe UI", Font.BOLD, 11));

        // Modelo
        JTextField modeloField = new JTextField(14);
        modeloField.setFont(lf);

        // Ubicación
        JTextField ubicacionField = new JTextField(14);
        ubicacionField.setFont(lf);

        // Estado
        JComboBox<String> estadoCombo = new JComboBox<>(
            new String[]{"activo", "inactivo", "en_reparacion", "dado_de_baja"});
        estadoCombo.setFont(lf);

        // Responsable: cargado desde usuarios activos (con opción vacía)
        JComboBox<UsuarioItem> responsableCombo = new JComboBox<>();
        responsableCombo.setFont(lf);
        responsableCombo.addItem(new UsuarioItem(-1, "— Sin responsable —"));
        try (Connection conn = getConnection();
             PreparedStatement ps = conn.prepareStatement(
                 "SELECT id, COALESCE(nombre, usuario) AS nombre FROM usuarios " +
                 "WHERE activo = 1 ORDER BY nombre")) {
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    responsableCombo.addItem(new UsuarioItem(rs.getInt("id"), rs.getString("nombre")));
                }
            }
        } catch (SQLException ex) {
            // Si falla la carga, queda solo la opción vacía
        }

        // Helper: calcula el próximo código según tipo
        java.util.function.Consumer<String> generarCodigo = (tipo) -> {
            String prefijo = PREFIJO_TIPO.getOrDefault(tipo, tipo.toUpperCase());
            String sqlMax = "SELECT COALESCE(MAX(CAST(SUBSTRING(codigo, LENGTH(?)+2) AS UNSIGNED)), 0) AS ultimo " +
                            "FROM equipos WHERE codigo LIKE CONCAT(?, '-%')";
            try (Connection conn = getConnection();
                 PreparedStatement ps = conn.prepareStatement(sqlMax)) {
                ps.setString(1, prefijo);
                ps.setString(2, prefijo);
                try (ResultSet rs = ps.executeQuery()) {
                    int siguiente = rs.next() ? rs.getInt("ultimo") + 1 : 1;
                    codigoField.setText(String.format("%s-%03d", prefijo, siguiente));
                }
            } catch (SQLException ex) {
                codigoField.setText(prefijo + "-001");
            }
        };

        // Generar código inicial
        generarCodigo.accept((String) tipoCombo.getSelectedItem());

        // Regenerar código al cambiar tipo
        tipoCombo.addActionListener(e -> generarCodigo.accept((String) tipoCombo.getSelectedItem()));

        // ── Layout del formulario ─────────────────────────────────────────────
        Object[][] filas = {
            {"Tipo de equipo: *",    tipoCombo},
            {"Código (asignado):",   codigoField},
            {"Modelo:",              modeloField},
            {"Ubicación:",           ubicacionField},
            {"Estado: *",            estadoCombo},
            {"Responsable:",         responsableCombo},
        };
        for (int i = 0; i < filas.length; i++) {
            ...
            form.add((java.awt.Component) filas[i][1], gc);
        }
        ...
        guardarBtn.setBackground(new Color(40, 140, 70));
        guardarBtn.setForeground(Color.WHITE);
        guardarBtn.setFocusPainted(false);
        ...
        guardarBtn.addActionListener(e -> {
            ...
            UsuarioItem resp  = (UsuarioItem) responsableCombo.getSelectedItem();
            Integer respId    = (resp != null && resp.getId() > 0) ? resp.getId() : null;

            if (codigo.isEmpty() || tipo.isEmpty()) {
                JOptionPane.showMessageDialog(dialog,
                    "El tipo es obligatorio para generar el código.",
                    "Campo requerido", JOptionPane.WARNING_MESSAGE);
                return;
            }

            // Re-verificar que el código no fue tomado entre tanto
            String sqlCheck = "SELECT COUNT(*) FROM equipos WHERE codigo = ?";
            try (Connection connChk = getConnection();
                 PreparedStatement psChk = connChk.prepareStatement(sqlCheck)) {
                psChk.setString(1, codigo);
                try (ResultSet rsChk = psChk.executeQuery()) {
                    if (rsChk.next() && rsChk.getInt(1) > 0) {
                        // Regenerar y avisar
                        generarCodigo.accept(tipo);
                        JOptionPane.showMessageDialog(dialog,
                            "El código " + codigo + " ya fue usado.\nSe asignó el siguiente disponible: " + codigoField.getText(),
                            "Código reasignado", JOptionPane.WARNING_MESSAGE);
                        return;
                    }
                }
            } catch (SQLException ex2) { /* continuar */ }

            String sql = "INSERT INTO equipos (codigo, tipo, modelo, ubicacion, estado, responsable_usuario_id) " +
                         "VALUES (?, ?, ?, ?, ?, ?)";
            try (Connection conn = getConnection();
                 PreparedStatement ps = conn.prepareStatement(sql)) {
                ps.setString(1, codigo);
                ...
                if (respId != null) ps.setInt(6, respId); else ps.setNull(6, Types.INTEGER);
                ps.executeUpdate();

                JOptionPane.showMessageDialog(dialog,
                    "Equipo \"" + codigo + "\" agregado correctamente.\nPuedes seleccionarlo ahora en la lista.",
                    "Equipo guardado", JOptionPane.INFORMATION_MESSAGE);
                dialog.dispose();
                // Recargar el combo y seleccionar el nuevo equipo
                cargarEquiposEnCombo();
                for (int i = 0; i < equipoComboEq.getItemCount(); i++) {
                    EquipoItem item = equipoComboEq.getItemAt(i);
                    if (item != null && codigo.equals(item.getCodigo())) {
                        equipoComboEq.setSelectedIndex(i);
                        break;
                    }
                }
            } catch (SQLException ex) {
    ...
    // =========================================================================
    // PANEL SOFTWARE (actualización de software en PCs)
    // =========================================================================

    private JPanel construirPanelSoftware() {
        ...
        softwareTableModel = new DefaultTableModel(
            new Object[]{"Software", "Versión anterior", "Versión nueva", "Observación"}, 0) {
            @Override public boolean isCellEditable(int r, int c) { return true; }
        };
        JTable softwareTable = new JTable(softwareTableModel);
        softwareTable.setFont(f);
        softwareTable.setRowHeight(24);
        softwareTable.getTableHeader().setFont(fb);
        softwareTable.getColumnModel().getColumn(0).setPreferredWidth(160);
        softwareTable.getColumnModel().getColumn(1).setPreferredWidth(110);
        softwareTable.getColumnModel().getColumn(2).setPreferredWidth(110);
        softwareTable.getColumnModel().getColumn(3).setPreferredWidth(220);

        JScrollPane tableScroll = new JScrollPane(softwareTable);
        tableScroll.setPreferredSize(new Dimension(600, 200));

        // Panel de agregar fila
        JPanel addRow = new JPanel(new FlowLayout(FlowLayout.LEFT, 6, 4));
        addRow.setOpaque(false);

        // Software: combo con opciones comunes + "Otro..."
        String[] softwareComunes = {
            "Windows 10", "Windows 11", "Microsoft Office", "Antivirus",
            "Google Chrome", "Mozilla Firefox", "Adobe Reader", "Java JRE",
            "Driver impresora", "Driver red / WiFi", "Otro..."
        };
        JComboBox<String> softwareCombo = new JComboBox<>(softwareComunes);
        softwareCombo.setEditable(false);
        softwareCombo.setFont(f);
        softwareCombo.setPreferredSize(new Dimension(160, 26));

        JTextField softwareCustomField = new JTextField(14);
        softwareCustomField.setFont(f);
        softwareCustomField.setToolTipText("Escribe el nombre del software");
        softwareCustomField.setVisible(false);

        // Mostrar/ocultar campo libre al elegir "Otro..."
        softwareCombo.addActionListener(e -> {
            boolean esOtro = "Otro...".equals(softwareCombo.getSelectedItem());
            softwareCustomField.setVisible(esOtro);
            addRow.revalidate();
            addRow.repaint();
        });

        JTextField vAnteriorField = new JTextField(8);
        vAnteriorField.setFont(f);
        vAnteriorField.setToolTipText("Versión anterior (ej: 21H2)");

        JTextField vNuevaField = new JTextField(8);
        vNuevaField.setFont(f);
        vNuevaField.setToolTipText("Versión nueva (ej: 23H2)");

        JTextField obsField = new JTextField(16);
        obsField.setFont(f);
        obsField.setToolTipText("Observación opcional");

        JButton addBtn = new JButton("+ Agregar");
        addBtn.setFont(fb);
        addBtn.setBackground(new Color(60, 130, 200));
        addBtn.setForeground(Color.WHITE);
        addBtn.setFocusPainted(false);

        addBtn.addActionListener(e -> {
            String nombreSw;
            if ("Otro...".equals(softwareCombo.getSelectedItem())) {
                nombreSw = softwareCustomField.getText().trim();
                if (nombreSw.isEmpty()) {
                    JOptionPane.showMessageDialog(panel, "Escribe el nombre del software.", "Campo requerido", JOptionPane.WARNING_MESSAGE);
                    return;
                }
            } else {
                nombreSw = (String) softwareCombo.getSelectedItem();
            }
            String vAnt  = vAnteriorField.getText().trim();
            String vNueva = vNuevaField.getText().trim();
            if (vNueva.isEmpty()) {
                JOptionPane.showMessageDialog(panel, "Indica al menos la versión nueva instalada.", "Campo requerido", JOptionPane.WARNING_MESSAGE);
                return;
            }
            softwareTableModel.addRow(new Object[]{nombreSw, vAnt, vNueva, obsField.getText().trim()});
            vAnteriorField.setText("");
            vNuevaField.setText("");
            obsField.setText("");
            softwareCustomField.setText("");
        });

        JButton removeBtn = new JButton("Eliminar fila");
        removeBtn.setFont(f);
        removeBtn.setFocusPainted(false);
        removeBtn.addActionListener(e -> {
            int row = softwareTable.getSelectedRow();
            if (row >= 0) softwareTableModel.removeRow(row);
        });

        addRow.add(new JLabel("Software:"));
        addRow.add(softwareCombo);
        addRow.add(softwareCustomField);
        addRow.add(new JLabel("Ver. anterior:"));
        addRow.add(vAnteriorField);
        addRow.add(new JLabel("Ver. nueva:"));
        addRow.add(vNuevaField);
        addRow.add(new JLabel("Obs:"));
        addRow.add(obsField);
        addRow.add(addBtn);
    ...
    private void limpiarFormularioEquipo() {
        // No se elimina la selección del equipo; solo se limpian los campos del mantenimiento.
        if (mantPreventivoBtnEq.isSelected()) {
            if (equipoActualEq != null) {
                cargarChecklistEquipo(equipoActualEq.getTipo());
            } else {
                checklistPanelEq.removeAll();
                checklistScrollEq.revalidate();
                checklistScrollEq.repaint();
                progresoBarra.setText("Progreso: 0/0 items [░░░░░░░░░░]");
                itemsCriticosLabel.setText("Selecciona un equipo primero");
                checklistActualEq = null;
            }
        } else if (mantCorrectivoBtnEq.isSelected()) {
            correctivoArea.setText("");
        } else if (mantSoftwareBtnEq.isSelected()) {
            if (softwareTableModel != null) {
                while (softwareTableModel.getRowCount() > 0)
                    softwareTableModel.removeRow(0);
            }
        }
    }

    private void cargarEquiposEnComboConsulta() {
        if (consultaEquipoComboEq == null) return;
        consultaEquipoComboEq.removeAllItems();
        consultaEquipoComboEq.addItem(new EquipoItem(-1, "— Selecciona un equipo —", "", "", "", "activo", ""));
        String sql = "SELECT e.id, e.codigo, e.tipo, e.modelo, e.ubicacion, e.estado, " +
                     "COALESCE(u.nombre, '-') AS responsable " +
                     "FROM equipos e " +
                     "LEFT JOIN usuarios u ON u.id = e.responsable_usuario_id " +
        ...
        try (Connection conn = getConnection();
             ...
             ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                consultaEquipoComboEq.addItem(new EquipoItem(
                    rs.getInt("id"), rs.getString("codigo"), rs.getString("tipo"),
                    rs.getString("modelo"), rs.getString("ubicacion"),
                    rs.getString("estado"), rs.getString("responsable")));
            }
        } catch (SQLException ex) {
            System.err.println("Error al cargar equipos en combo consulta: " + ex.getMessage());
        }
    }

    private void seleccionarEquipoDesdeComboConsulta() {
        EquipoItem equipo = (EquipoItem) consultaEquipoComboEq.getSelectedItem();
        if (equipo == null || equipo.getId() == -1) {
            consultaEquipoInfoLabelEq.setText("Selecciona un equipo de la lista para consultar sus mantenciones.");
            consultaEquipoInfoLabelEq.setForeground(Color.DARK_GRAY);
        ...
        consultaEquipoInfoLabelEq.setText("✓ " + equipo.getCodigo() + " | " + equipo.getTipo() + " | " + equipo.getModelo());
